...
class Clementine
{
    ...
    public function run()
    {
        ...
        $erreur_404 = 0;
        // initialise le tableau Clementine::$register['request'], et supprime les slashes rajoutes par magic_quotes_gpc
        if (get_magic_quotes_gpc()) {
            Clementine::$register['request'] = array(
                'GET'     => $this->stripslashes_recursive($_GET),
                'POST'    => $this->stripslashes_recursive($_POST),
                'COOKIE'  => $this->stripslashes_recursive($_COOKIE),
                'REQUEST' => $this->stripslashes_recursive($_REQUEST));
        } else {
            Clementine::$register['request'] = array(
                'GET'     => $_GET,
                'POST'    => $_POST,
                    ...
                    if (!class_exists($current_class . '_Parent', false)) {
                        ...
                        if (defined($adopter)) {
                            if (!class_exists(constant($adopter), false)) {
                                ...
                                $this->_factory(substr(constant($adopter), 0, - strlen($type)), $type, $testonly);
                            }
     ...
     * @return void
     */
    public function getBlock($path, $data = null, $ignores = null, $load_parent = false, $testonly = false)
    {
     ...
     * @return void
     */
    public function getParentBlock($data = null, $ignores = null)
    {
     ...
     * @return void
     */
    public function getBlockHtml($path, $data = null, $ignores = null, $load_parent = false)
    {
        ob_start();
     ...
     * @return void
     */
    public function canGetBlock($path, $data = null, $ignores = null, $load_parent = false)
    {
    ...
    */
    ...
    /**
     * stripslashes_recursive : recursive stripslashes
     * 
     * @param mixed $array 
     * @access public
     * @return void
     */
    public function stripslashes_recursive($array) {
        $new = array();
        foreach($array as $key => $value) {
            $key = stripslashes($key);
            if (is_array($value)) {
                $new[$key] = $this->stripslashes_recursive($value);
            }
            else {
