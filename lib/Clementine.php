...
class Clementine
{
    ...
    public function __call($name, $args)
    {
        ...
        if (!$call_parent) {
            if (__DEBUGABLE__) {
                echo "<br />\n" . '<strong>Clementine fatal error</strong>: Call to undefined method ::' . $name . ' in ' . ' on line ';
            }
            die();
        }
    }
    ...
    public function run()
    {
        ...
        if (!$controller) {
            ...
            if (count((array) $controller)) {
                ...
                if (method_exists($controller, $action)) {
                    $noblock = $controller->$action();
                } else {
                    header($_SERVER['SERVER_PROTOCOL'] . ' 404 Not Found', true);
                    $erreur_404 = 1;
                    if (__DEBUGABLE__) {
                        $this->getController('debug')->err404_noSuchMethod();
                    }
                }
            } else {
                header($_SERVER['SERVER_PROTOCOL'] . ' 404 Not Found', true);
                $erreur_404 = 1;
                if (__DEBUGABLE__) {
                    $this->getController('debug')->err404_cannotLoadCtrl();
                }
            }
        }
        ...
        if (!$erreur_404 && !$noblock) {
            ...
            $gotblock = false;
            if ($controller->canGetBlock($path)) {
                $gotblock = $controller->getBlock($path, $controller->data);
            } else {
                $adopter = '__CLEMENTINE_CLASS_' . strtoupper($request['CTRL']) . '_VIEW_EXTENDS__';
                if (defined($adopter)) {
                    // le block n'a pas de parent mais il est adopte
                    $tuteur_path = substr(strtolower(constant($adopter)), 0, -4) . '/' . $request['ACT']; // strips de "view" part
                    $gotblock = $controller->getBlock($tuteur_path, $controller->data);
                }
            }
            if (!$gotblock) {
                header($_SERVER['SERVER_PROTOCOL'] . ' 404 Not Found', true);
                $erreur_404 = 1;
                if (__DEBUGABLE__) {
                    $this->getController('debug')->err404_noSuchBlock();
                }
            }
        }
        ...
        if (__DEBUGABLE__) {
            $this->getController('debug')->generationTime($mvc_generation_begin, microtime(true));
        }
    }
     ...
     * @return void
     */
    public function trigger404($path = null)
    {
        if ($path) {
            $this->getController('debug')->trigger_error("Erreur 404 : le bloc " . $path . " est introuvable ou a renvoye une erreur", E_USER_WARNING, -1);
        } else {
            $controller = $this->getController('errors');
            $action = 'err404Action';
            $controller->$action();
            if (!$controller->getBlock('errors/err404', $controller->data)) {
                if (__DEBUGABLE__) {
                    $this->getController('debug')->err404_no404Block();
                }
                echo '404 Not Found';
            }
        }
    ...
    public function hook($hookname, $args = null)
    {
        ...
        $was_called = false;
        if (method_exists($ctrl, $hookname)) {
            $was_called = true;
            $this->getController('hook')->$hookname($args);
        }
        if (__DEBUGABLE__) {
            $this->getController('debug')->debugHook($hookname, $was_called);
        }
    }
    ...
    {
        if (!(isset(Clementine::$register['request']) && Clementine::$register['request'])) {
            ...
            if ((count($lang_dispos) > 1) && $lang_candidat && in_array($lang_candidat, $lang_dispos)) {
                ...
                if ((count($lang_dispos) > 1) && strlen($lang_candidat)) {
                    header($_SERVER['SERVER_PROTOCOL'] . ' 404 Not Found', true);
                    if (__DEBUGABLE__) {
                        $this->getController('debug')->err404_noLanguageCode();
                    }
                    $request['CTRL'] = 'errors';
                    $request['ACT'] = 'err404';
                }
     ...
     * @return void
     */
    private function _factory($element, $type)
    {
        switch ($type) {
            case 'Model':
                $type_short = 'model';
        ...
        if (!class_exists($elementname, false)) {
                    ...
                    if (!class_exists($current_class . '_Parent', false)) {
                        ...
                        if (defined($adopter)) {
                            if (!class_exists(constant($adopter), false)) {
                                ...
                                $this->_factory(substr(constant($adopter), 0, - strlen($type)), $type);
                            }
            ...
            if (isset($prev) && class_exists($prev . $elementname, false)) {
                ...
                if ($type == 'Controller') {
                    if (__DEBUGABLE__) {
                        $this->getController('debug')->err404_noSuchController($elementname);
                    }
                    return false;
                } else {
                    // erreur fatale : on a demande a charger un modele qui n'existe pas
                    if (__DEBUGABLE__) {
                        $this->getController('debug')->errFatale_noSuchModel($type, $element);
                    }
                    die();
                }
            }
        }
        $new_element = new $elementname();
        if (__DEBUGABLE__) {
    ...
    {
        return $this->_factory($model, 'Model');
    }
    ...
    {
        return $this->_factory($ctrl, 'Controller');
    }
     ...
     * @return void
     */
    public function getBlock($path, $data = null, $ignores = array(), $load_parent = false, $testonly = false)
    {
        $path = strtolower($path);
        if (__DEBUGABLE__) {
            $this->getController('debug')->debugBlock_init();
        }
        ...
        foreach ($reverse as $module => $scope) {
            ...
            $file = __FILES_ROOT__ . '/app/' . $scope . '/' . $module . '/view/' . $path_array[0] . '/' . $path_array[1] . '.php';
            ...
            if ($block_exists) {
                ...
                if ($load_block) {
                    ...
                    if (__DEBUGABLE__) {
                        $this->getController('debug')->debugBlock_register_stack($scope, $module, $path, $file, $ignores, $load_parent);
                    }
                    ...
                    if (__DEBUGABLE__) {
                        $this->getController('debug')->debugBlock_warningRecursiveCall($path);
                    }
                    $vue_recursive = 1;
                    break;
                }
        ...
        if (!$vue_affichee && !$vue_recursive) {
            ...
            if (__DEBUGABLE__) {
                $this->getController('debug')->debugBlock_dumpStack($scope, $module, $path_array);
            }
        }
        if (!$found && $load_parent) {
            ...
            if (defined($adopter)) {
                ...
                $found = $this->getBlock($tuteur_path, $data, $ignores, $load_parent, $testonly);
            }
        }
        return $found;
    }
     ...
     * @return void
     */
    public function canGetBlock($path, $data = null, $ignores = array(), $load_parent = false)
    {
        return $this->getBlock($path, $data, $ignores, $load_parent, true);
    }
     ...
     * @return void
     */
    public function getParentBlock($data = null, $ignores = array())
    {
        $parent_blocks = Clementine::$_register['_parent_loaded_blocks'];
        $last_block = array_pop($parent_blocks);
        if ($last_block) {
            return $this->getBlock($last_block, $data, $ignores, true);
        } else {
            return 0;
        }
    }
     ...
     * @return void
     */
    public function getBlockHtml($path, $data = null, $ignores = array(), $load_parent = false)
    {
        ob_start();
        $this->getBlock($path, $data, $ignores, $load_parent);
    ...
    public function apply_config()
    {
        ...
        if (__DEBUGABLE__ && Clementine::$config['clementine_debug']['overrides']) {
            $message = '<table width="300" style="font-size: 12px; line-height: 1.4em; text-align: left; "><thead><th>module</th><th>version</th><th>type</th><th>poids</th></thead><tbody>';
    ...
    public function debug()
    {
        ...
        if (__DEBUGABLE__ && !$request['AJAX'] && !defined('__NO_DEBUG_DIV__')) {
            ...
            echo '<div id="Clementine_debug_div" style="background: #EEE; font-family: courier; font-size: 14px; padding: 0.5em; -moz-border-radius: 5px; " >';
            echo '<div style="text-align: right; ">';
            echo '<strong>DEBUG</strong>';
            echo '<span style="cursor: pointer" onclick=\'';
            echo 'document.getElementById("Clementine_debug_ol").style.display = (parseInt(document.cookie.substring(parseInt("Clementine_debug_div_hide".length) + document.cookie.indexOf("Clementine_debug_div_hide") + 1, parseInt("Clementine_debug_div_hide".length) + document.cookie.indexOf("Clementine_debug_div_hide") + 2)) ? "block" : "none"); ';
            echo 'document.cookie="Clementine_debug_div_hide=" + escape(parseInt(document.cookie.substring(parseInt("Clementine_debug_div_hide".length) + document.cookie.indexOf("Clementine_debug_div_hide") + 1, parseInt("Clementine_debug_div_hide".length) + document.cookie.indexOf("Clementine_debug_div_hide") + 2)) ? "0" : "1") + "; ';
            echo 'path=' . __BASE_URL__ . '/"\'>[toggle]</span></div>';
            echo '<ol id="Clementine_debug_ol" style="';
            echo 'text-align: left; ';
            echo 'padding: 0.5em 1em; ';
            echo 'margin: 0; ';
            echo 'list-style-position: inside; ';
            echo (isset($_COOKIE['Clementine_debug_div_hide']) && ($_COOKIE['Clementine_debug_div_hide'])) ? 'display: none; ' : 'display: block; ';
            echo ' ">';
            // if (isset(Clementine::$clementine_debug['sql'])) {
            foreach ($types as $type => $libelle) {
                if (isset(Clementine::$clementine_debug[$type]) && count(Clementine::$clementine_debug[$type])) {
                    echo '<li style="margin: 3px; border: solid #AAA 3px; padding: 3px; -moz-border-radius: 5px; background-color: #BBB; font-size: 12px; line-height: 1.4em; z-index: 9998">';
                    echo '<strong>' . $libelle . '</strong>';
                    echo '<ol>';
                    if (isset(Clementine::$clementine_debug[$type])) {
                        ...
                        foreach (Clementine::$clementine_debug[$type] as $msg) {
                            ...
                            if ($type == 'sql') {
                                ...
                                $msg['duree'] = '<strong>Durée : </strong>' . number_format($msg['duree'], 3, ',', ' ') . ' ms';
                            }
                            echo '<li style="background-color: #DDD; margin: 3px; padding: 5px; -moz-border-radius: 5px; border: dotted #CCC 1px; ">';
                            if (is_array($msg)) {
                        ...
                        if ($type == 'sql') {
                            echo '<li style="background-color: #DDD; margin: 3px; padding: 5px; -moz-border-radius: 5px; border: dotted #CCC 1px; ">';
                            echo '<strong>Durée totale passé en query (hors fetch) : </strong>' . number_format($duree_totale_sql, 3, ',', ' ') . ' ms';
                            echo '</li>';
                        }
                    }
                    echo '</ol>';
