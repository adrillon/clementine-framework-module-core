...
class Clementine
{
    ...
    public function run()
    {
        ...
        if (!$controller) {
            ...
            if (count((array) $controller)) {
                ...
                if (method_exists($controller, $action)) {
                    ...
                    $erreur_404 = 1;
                    if (__DEBUGABLE__) {
                        $this->getHelper('debug')->err404_noSuchMethod();
                    }
                }
            } else {
                header($_SERVER['SERVER_PROTOCOL'] . ' 404 Not Found', true);
                $erreur_404 = 1;
                if (__DEBUGABLE__) {
                    $this->getHelper('debug')->err404_cannotLoadCtrl();
                }
            }
        }
        ...
        if ($erreur_404) {
            if (__DEBUGABLE__) {
                $this->getHelper('debug')->err404_noSuchBlock();
            }
            $this->trigger404();
        }
        if (__DEBUGABLE__) {
            $this->getHelper('debug')->generationTime($mvc_generation_begin, microtime(true));
        }
    }
    ...
    {
        return $this->_factory($helper, 'Helper', $params);
    }
        ...
        foreach ($reverse as $module => $scope) {
            ...
            if ($block_exists) {
                ...
                if ($load_block) {
                    ...
                    if (__DEBUGABLE__) {
                        $this->getHelper('debug')->debugBlock_register_stack($scope, $module, $path, $file, $ignores, $load_parent);
                    }
